app.service('FromCarousel', function(Uts) {
    var self = this;
    this.currdeg = 0;

    this.onStartUp = function(){
        $("#itemFrom" + this.currdeg).css({
    ...
    this.getVisibleCarouselValues = function(){
        ...
        var caroCount = ($(".carousel-from")[0].children).length;
        for(var i = caroCount - 1; i >= 0; i-- ){
            var id = ($(".carousel-from")[0].children)[i].id;
            var idRot = Math.abs(id.replace("itemFrom",""));

            if((rot === 0 && rot === idRot) || (rot !== 0 && rot % idRot == 0)) {
                vis.currentID = id;
                if (i == caroCount - 1) {
                    vis.leftID = ($(".carousel-from")[0].children)[i - 1].id;
                    vis.rightID = ($(".carousel-from")[0].children)[0].id;
                } else if (i == 0) {
                    vis.leftID = ($(".carousel-from")[0].children)[caroCount - 1].id;
                    vis.rightID = ($(".carousel-from")[0].children)[i + 1].id;
                } else {
                    vis.leftID = ($(".carousel-from")[0].children)[i - 1].id;
                    vis.rightID = ($(".carousel-from")[0].children)[i + 1].id;
                }
