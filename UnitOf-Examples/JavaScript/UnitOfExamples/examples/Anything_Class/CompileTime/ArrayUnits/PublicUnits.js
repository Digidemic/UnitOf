...
(function() {
     ...
     */
    arr_PublicUnits.FEET = 1;           //Unit "Feet" is the key for all conversions here. All added units are their conversion into 1 foot.
    arr_PublicUnits.METERS = 0.3048;    //Unit name/value: 0.3048 Meters in 1 foot.
    arr_PublicUnits.INCHES = 12;        //Unit name/value: 12 inches in 1 foot.

    /*
     Only function. First available which is used to set the "from" value and unit.
     Once set, inner "to" function is available
     */
    arr_PublicUnits.from = function(unit, input){
         ...
         */
        var CUSTOM_LENGTH = UnitOf.Anything(arr_PublicUnits.FEET, 1, true, [
            arr_PublicUnits.FEET,
            arr_PublicUnits.METERS,
            arr_PublicUnits.INCHES
        ]);
 ...
 */
var anythingArrayPublicUnits = function(){
    var results = '';

    p("\n-=-=-=-=-=-=-=-=-\nAnything_Class > CompileTime > ArrayUnits > PublicUnits");
    p("-=-=-=-=-\nEx 1 of 2: 5 meters is equal to:");
    var ex1 = arr_PublicUnits.from(arr_PublicUnits.METERS, 5);  //Sets ex1 to 5 meters
    pt(ex1.to(arr_PublicUnits.FEET) + " feet");      //Returns 16.404 as 5 meters to feet = ~16.404 feet
    pt(ex1.to(arr_PublicUnits.METERS) + " meters");  //Returns 5 as 5 meters to meters = 5 meters
    pt(ex1.to(arr_PublicUnits.INCHES) + " inches");  //Returns 196.85 as 5 meters to inches = ~196.85 inches

    p("-=-=-=-=-\nEx 2 of 2: 12.5 feet is equal to:");
    var ex2 = arr_PublicUnits.from(arr_PublicUnits.FEET, 12.5); //Sets ex2 to 12.5 feet
    pt(ex2.to(arr_PublicUnits.FEET) + " feet");      //Returns 12.5 as 12.5 feet to feet = 12.5 feet
    pt(ex2.to(arr_PublicUnits.METERS) + " meters");  //Returns 3.81 as 12.5 feet to meters = 3.81 meters
    pt(ex2.to(arr_PublicUnits.INCHES) + " inches");  //Returns 150 as 12.5 feet to inches = 150 inches
